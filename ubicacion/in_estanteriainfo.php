...
//
// Table class for in_estanteria
//
class cin_estanteria extends cTable {
	var $Estanteria_Id;
	var $descripcion;
	var $es_muestra;
	...
	function __construct() {
		...
		$this->TableVar = 'in_estanteria';
		$this->TableName = 'in_estanteria';
		...
		// Estanteria_Id
		$this->Estanteria_Id = new cField('in_estanteria', 'in_estanteria', 'x_Estanteria_Id', 'Estanteria_Id', '[Estanteria_Id]', 'CAST([Estanteria_Id] AS NVARCHAR)', 3, -1, FALSE, '[Estanteria_Id]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->Estanteria_Id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['Estanteria_Id'] = &$this->Estanteria_Id;

		// descripcion
		$this->descripcion = new cField('in_estanteria', 'in_estanteria', 'x_descripcion', 'descripcion', '[descripcion]', '[descripcion]', 200, -1, FALSE, '[descripcion]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['descripcion'] = &$this->descripcion;

		// es_muestra
		$this->es_muestra = new cField('in_estanteria', 'in_estanteria', 'x_es_muestra', 'es_muestra', '[es_muestra]', '[es_muestra]', 11, -1, FALSE, '[es_muestra]', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->es_muestra->FldDataType = EW_DATATYPE_BOOLEAN;
		$this->fields['es_muestra'] = &$this->es_muestra;
	}
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "[dbo].[in_estanteria]";
	}
	...
	function getSqlOrderBy() { // Order By
		return ($this->_SqlOrderBy <> "") ? $this->_SqlOrderBy : "[Estanteria_Id] ASC";
	}
	...
	// Update Table
	var $UpdateTable = "[dbo].[in_estanteria]";
	...
	function SqlKeyFilter() {
		return "[Estanteria_Id] = @Estanteria_Id@";
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("in_estanteriaview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("in_estanteriaview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}

	// Add URL
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			return "in_estanteriaadd.php?" . $this->UrlParm($parm);
		else
			return "in_estanteriaadd.php";
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		return $this->KeyUrl("in_estanteriaedit.php", $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl("in_estanteriaadd.php", $this->UrlParm($parm));
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("in_estanteriadelete.php", $this->UrlParm());
	}
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$arKeys[] = @$_GET["Estanteria_Id"]; // Estanteria_Id
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->Estanteria_Id->CurrentValue = $key;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->es_muestra->Exportable) $Doc->ExportCaption($this->es_muestra);
				} else {
					if ($this->Estanteria_Id->Exportable) $Doc->ExportCaption($this->Estanteria_Id);
					if ($this->descripcion->Exportable) $Doc->ExportCaption($this->descripcion);
					if ($this->es_muestra->Exportable) $Doc->ExportCaption($this->es_muestra);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->es_muestra->Exportable) $Doc->ExportField($this->es_muestra);
					} else {
						if ($this->Estanteria_Id->Exportable) $Doc->ExportField($this->Estanteria_Id);
						if ($this->descripcion->Exportable) $Doc->ExportField($this->descripcion);
						if ($this->es_muestra->Exportable) $Doc->ExportField($this->es_muestra);
