...
$in_bodegaubicaciones_grid = NULL; // Initialize page object first

class cin_bodegaubicaciones_grid extends cin_bodegaubicaciones {
	...
	// Page object name
	var $PageObjName = 'in_bodegaubicaciones_grid';

	// Grid form hidden field names
	var $FormName = 'fin_bodegaubicacionesgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["in_bodegaubicaciones"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->codigo_bodega->CurrentValue;
					if ($sKey <> "") $sKey .= $GLOBALS["EW_COMPOSITE_KEY_SEPARATOR"];
					$sKey .= $this->Estanteria_Id->CurrentValue;
					if ($sKey <> "") $sKey .= $GLOBALS["EW_COMPOSITE_KEY_SEPARATOR"];
					$sKey .= $this->Seccion_id->CurrentValue;
	...
	function EmptyRow() {
		global $objForm;
		if ($objForm->HasValue("x_codigo_bodega") && $objForm->HasValue("o_codigo_bodega") && $this->codigo_bodega->CurrentValue <> $this->codigo_bodega->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_Estanteria_Id") && $objForm->HasValue("o_Estanteria_Id") && $this->Estanteria_Id->CurrentValue <> $this->Estanteria_Id->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_Seccion_id") && $objForm->HasValue("o_Seccion_id") && $this->Seccion_id->CurrentValue <> $this->Seccion_id->OldValue)
			return FALSE;
		return TRUE;
	}
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->codigo_bodega->CurrentValue . $GLOBALS["EW_COMPOSITE_KEY_SEPARATOR"] . $this->Estanteria_Id->CurrentValue . $GLOBALS["EW_COMPOSITE_KEY_SEPARATOR"] . $this->Seccion_id->CurrentValue . "\">";
	...
	function SetRecordKey(&$key, $rs) {
		...
		$key .= $rs->fields('codigo_bodega');
		if ($key <> "") $key .= $GLOBALS["EW_COMPOSITE_KEY_SEPARATOR"];
		$key .= $rs->fields('Estanteria_Id');
		if ($key <> "") $key .= $GLOBALS["EW_COMPOSITE_KEY_SEPARATOR"];
		$key .= $rs->fields('Seccion_id');
	}
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 3) {
			if (strval($arKeys[0]) <> "")
				$this->codigo_bodega->CurrentValue = strval($arKeys[0]); // codigo_bodega
			else
				$bValidKey = FALSE;
			if (strval($arKeys[1]) <> "")
				$this->Estanteria_Id->CurrentValue = strval($arKeys[1]); // Estanteria_Id
			else
				$bValidKey = FALSE;
			if (strval($arKeys[2]) <> "")
				$this->Seccion_id->CurrentValue = strval($arKeys[2]); // Seccion_id
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "in_bodegas") {
			...
			if ($GLOBALS["in_bodegas"]->EventCancelled) $this->EventCancelled = TRUE;
		}
