...
$in_estanteria_add = NULL; // Initialize page object first

class cin_estanteria_add extends cin_estanteria {
	...
	// Page object name
	var $PageObjName = 'in_estanteria_add';
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if (ew_GetPageName($sReturnUrl) == "in_estanteriaview.php")
	...
	function LoadDefaultValues() {
		...
		$this->es_muestra->CurrentValue = NULL;
		$this->es_muestra->OldValue = $this->es_muestra->CurrentValue;
	}
	...
	function AddRow($rsold = NULL) {
		...
		if ($this->Estanteria_Id->CurrentValue <> "") { // Check field with unique index
			$sFilter = "(Estanteria_Id = " . ew_AdjustSql($this->Estanteria_Id->CurrentValue) . ")";
			$rsChk = $this->LoadRs($sFilter);
			if ($rsChk && !$rsChk->EOF) {
				$sIdxErrMsg = str_replace("%f", $this->Estanteria_Id->FldCaption(), $Language->Phrase("DupIndex"));
				$sIdxErrMsg = str_replace("%v", $this->Estanteria_Id->CurrentValue, $sIdxErrMsg);
		...
		$this->es_muestra->SetDbValueDef($rsnew, $tmpBool, NULL, FALSE);
...
// Create page object
if (!isset($in_estanteria_add)) $in_estanteria_add = new cin_estanteria_add();

// Page init
$in_estanteria_add->Page_Init();

// Page main
$in_estanteria_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$in_estanteria_add->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var in_estanteria_add = new ew_Page("in_estanteria_add");
in_estanteria_add.PageID = "add"; // Page ID
var EW_PAGE_ID = in_estanteria_add.PageID; // For backward compatibility

// Form object
var fin_estanteriaadd = new ew_Form("fin_estanteriaadd");

// Validate form
fin_estanteriaadd.Validate = function() {
...
fin_estanteriaadd.Form_CustomValidate = 
...
fin_estanteriaadd.ValidateRequired = true;
<?php } else { ?>
fin_estanteriaadd.ValidateRequired = false; 
<?php } ?>
...
<?php $in_estanteria_add->ShowPageHeader(); ?>
<?php
$in_estanteria_add->ShowMessage();
?>
<form name="fin_estanteriaadd" id="fin_estanteriaadd" class="form-horizontal ewForm ewAddForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($in_estanteria_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $in_estanteria_add->Token ?>">
<?php } ?>
...
<input type="text" data-field="x_Estanteria_Id" name="x_Estanteria_Id" id="x_Estanteria_Id" size="30" placeholder="<?php echo ew_HtmlEncode($in_estanteria->Estanteria_Id->PlaceHolder) ?>" value="<?php echo $in_estanteria->Estanteria_Id->EditValue ?>"<?php echo $in_estanteria->Estanteria_Id->EditAttributes() ?>>
</span>
<?php } else { ?>
<span id="el_in_estanteria_Estanteria_Id">
...
<script type="text/javascript">
fin_estanteriaadd.Init();
</script>
<?php
$in_estanteria_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$in_estanteria_add->Page_Terminate();
?>
